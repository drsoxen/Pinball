...
Model3D::Model3D(ID3D10Device* device)
{
	memset(this, 0, sizeof(Model3D));
...
void Model3D::Init()
{
	if(!beenInitted)
	{
		...
		{
			LineModel3D * line = new LineModel3D(m_Device);
...
void Model3D::DrawNormals(D3DXMATRIX WorldMatrix,D3DXMATRIX ViewMatrix,D3DXMATRIX ProjectionMatrix,float DeltaTime)
{
	for(int i = 0; i < debugNormals.size(); i++)
		debugNormals[i]->Draw(WorldMatrix,ViewMatrix,ProjectionMatrix,DeltaTime);
}

void Model3D::SetPosition(Vector3 Position)
{
...
void Model3D::SetRotation(Vector3 Rotation)
{
...
Vector3 Model3D::GetRotation()
{
	return m_Rotation;
}

//void Model3D::SetBallPositionDependent( Ball* m_ball )
//{
//	m_BallDependent = m_ball;
//}

void Model3D::ChangeOrigin( Vector3 NewOrigin )
{
